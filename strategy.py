...
def check_5m_smc_golden_fibo_sniper(pair: str, candles_5m: list) -> dict:
    """
    Evaluates 5-Minute SMC Golden Fibonacci Sniper Strategy (5m Chart -> Next 5m Candle Expiry).
    Uses pure Market Structure (Pivot Highs/Lows) to determine the impulse leg (BOS/CHoCH direction).
    Draws Fibonacci from the impulse leg and waits for a retracement into the 0.5 - 0.618 Golden Zone.
    Triggers when a reversal candle closes inside/at the Golden Zone.
    """
    ...
    # 1. Identify Pivot Highs and Pivot Lows (Structure)
    left_bars = 4
    right_bars = 4
    
    all_pivots = []
    
    # Stop before -2 to ignore the forming candle and just-closed candle for pivot confirmation
    for i in range(left_bars, len(df) - right_bars - 1):
        c_high = float(df.iloc[i]['high'])
        c_low = float(df.iloc[i]['low'])
        
        is_ph = True
        is_pl = True
        
        for j in range(1, left_bars + 1):
            if float(df.iloc[i-j]['high']) >= c_high: is_ph = False
            if float(df.iloc[i-j]['low']) <= c_low: is_pl = False
        ...
        if is_ph:
            all_pivots.append({'type': 'PH', 'idx': i, 'val': c_high})
        if is_pl:
            all_pivots.append({'type': 'PL', 'idx': i, 'val': c_low})
    ...
    # 2. Get the last impulse leg
    last_pivot = all_pivots[-1]
    prev_pivot = all_pivots[-2]
    
    # If the last two pivots are of the same type, find the most recent alternating pivots
    if last_pivot['type'] == prev_pivot['type']:
        # Fallback: Just find the absolute highest and lowest in the last 30 bars
        recent_df = df.iloc[-32:-2]
        h_idx = recent_df['high'].idxmax()
        l_idx = recent_df['low'].idxmin()
        if h_idx > l_idx:
            prev_pivot = {'type': 'PL', 'idx': l_idx, 'val': float(recent_df.loc[l_idx, 'low'])}
            last_pivot = {'type': 'PH', 'idx': h_idx, 'val': float(recent_df.loc[h_idx, 'high'])}
        else:
            prev_pivot = {'type': 'PH', 'idx': h_idx, 'val': float(recent_df.loc[h_idx, 'high'])}
            last_pivot = {'type': 'PL', 'idx': l_idx, 'val': float(recent_df.loc[l_idx, 'low'])}
            
    # 3. Get the candle that just closed
    idx = len(df) - 2
    c = df.iloc[idx]
    p = df.iloc[idx - 1]
    ...
    # We optionally can use volume spike logic, but let's keep it pure SMC price action for now
    
    signal = None
    
    # 4. UPWARD IMPULSE LEG (Looking for CALL at Fibo retracement)
    if prev_pivot['type'] == 'PL' and last_pivot['type'] == 'PH':
        swing_low = prev_pivot['val']
        swing_high = last_pivot['val']
    ...
    # 5. DOWNWARD IMPULSE LEG (Looking for PUT at Fibo retracement)
        ...
        swing_high = prev_pivot['val']
        swing_low = last_pivot['val']
        diff = swing_high - swing_low
    ...
    if signal:
        return {
            ...
            "volume": float(c.get('volume', 1.0)),
            "epoch": c_epoch,
            "strategy_name": "5m SMC Golden Fibo Sniper (5m Expiry)"
        }
